...
<style>
:root{
  --bg:#0a0e14;--sidebar:#0d1117;--card:#111820;
...
@keyframes pulse{0%,100%{box-shadow:0 0 0 0 rgba(63,185,80,.5);}50%{box-shadow:0 0 0 4px rgba(63,185,80,0);}}
...
.hero-card{border-radius:var(--r);padding:20px 22px;position:relative;overflow:hidden;border:1px solid var(--line);}
.hero-card.green{background:linear-gradient(135deg,var(--green-dim),var(--card));border-color:rgba(63,185,80,.2);}
.hero-card.blue{background:linear-gradient(135deg,var(--blue-dim),var(--card));border-color:rgba(88,166,255,.2);}
.hero-card.neutral{background:var(--card);}
.hero-glow{position:absolute;top:-30px;right:-30px;width:90px;height:90px;border-radius:50%;opacity:.15;filter:blur(30px);}
.hero-card.green .hero-glow{background:var(--green);}
.hero-card.blue  .hero-glow{background:var(--blue);}
.hero-icon{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;margin-bottom:14px;}
...
.hero-label{font-size:11px;text-transform:uppercase;letter-spacing:.8px;color:var(--muted);font-weight:500;margin-bottom:6px;}
...
.ring-label{position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;color:var(--blue);}
.spark-wrap{margin-top:10px;}
.spark-wrap svg{width:100%;height:28px;display:block;}
.spark-wrap polyline{fill:none;stroke:var(--green);stroke-width:1.5;stroke-linejoin:round;stroke-linecap:round;opacity:.8;}
.section-head{display:flex;align-items:center;gap:10px;margin-bottom:14px;margin-top:28px;}
.section-head:first-child{margin-top:0;}
.section-head h2{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.8px;color:var(--muted2);white-space:nowrap;}
...
th{background:var(--bg);color:var(--muted2);font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.5px;padding:10px 16px;border-bottom:1px solid var(--line);text-align:left;}
...
.gw-hero{display:grid;grid-template-columns:repeat(3,1fr);gap:14px;margin-bottom:28px;}
</style>
</head>
<body>
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="page-section visible" id="sec-overview">
      <div class="hero">
        <div class="hero-card green">
          ...
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3fb950" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg></div>
          <div class="hero-label">Tokens saved</div>
          <div class="hero-value" id="ov-tokens">—</div>
          <div id="ov-tokens-trend" class="hero-trend" style="visibility:hidden;color:var(--green)"> </div>
          <div class="spark-wrap"><svg viewBox="0 0 120 28" preserveAspectRatio="none"><polyline id="sl-tokens" points=""/></svg></div>
        </div>
        <div class="hero-card green">
          <div class="hero-glow"></div>
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3fb950" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></div>
          <div class="hero-label">Cost saved</div>
          <div class="hero-value" id="ov-cost">—</div>
          <div class="hero-sub" id="ov-cost-sub"></div>
        </div>
        <div class="hero-card neutral">
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7a8799" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div>
          <div class="hero-label">Requests</div>
          <div class="hero-value" id="ov-requests">—</div>
          <div class="hero-sub" id="ov-compressed"></div>
          <div class="bar-wrap"><div class="bar" id="ov-req-bar" style="width:0%;background:var(--blue)"></div></div>
        </div>
        <div class="hero-card blue">
          <div class="hero-glow"></div>
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#58a6ff" stroke-width="2"><polyline points="22 8 22 16"/><polyline points="2 8 2 16"/><path d="M5 5h14l2 3-9 4-9-4 2-3Z"/></svg></div>
          <div class="hero-label">Avg compression</div>
          <div class="ring-wrap">
            <div class="ring">
              <svg width="56" height="56" viewBox="0 0 56 56"><circle class="ring-bg" cx="28" cy="28" r="22"/><circle class="ring-fill" id="ov-ring-arc" cx="28" cy="28" r="22"/></svg>
              <div class="ring-label" id="ov-ring-pct">0%</div>
            </div>
            <div><div class="hero-value" id="ov-ratio">—</div><div class="hero-sub">smaller</div></div>
          </div>
        </div>
      </div>
      <div class="section-head"><h2>Quality snapshot</h2></div>
      <div class="q-grid">
        <div class="q-card">
          <div class="q-title">ROUGE-1 quality (A/B)</div>
          <div class="q-score hi" id="ov-quality">—</div>
          <div class="hero-sub">mean across samples</div>
          <div class="q-bar"><div class="q-bar-fill" id="ov-quality-bar" style="width:0%"></div></div>
        </div>
        <div class="q-card">
          <div class="q-title">Numeric consistency</div>
          <div class="q-score hi" id="ov-numeric">—</div>
          <div class="hero-sub">exact value preservation</div>
          <div class="q-bar"><div class="q-bar-fill" id="ov-numeric-bar" style="width:0%"></div></div>
        </div>
      </div>
      <div class="section-head"><h2>MCP gateway · quick view</h2></div>
      <div class="gw-hero">
        <div class="hero-card green">
          <div class="hero-glow"></div>
          <div class="hero-label">Gateway tokens saved</div>
          <div class="hero-value" id="ov-g-tokens">—</div>
          <div class="spark-wrap"><svg viewBox="0 0 120 28" preserveAspectRatio="none"><polyline id="sl-gtokens" points=""/></svg></div>
        </div>
        <div class="hero-card neutral">
          <div class="hero-label">Tool calls</div>
          <div class="hero-value" id="ov-g-calls">—</div>
          <div class="hero-sub" id="ov-g-calls-sub"></div>
        </div>
        <div class="hero-card blue">
          <div class="hero-glow"></div>
          <div class="hero-label">Gateway compression</div>
          <div class="hero-value" id="ov-g-ratio">—</div>
        </div>
      </div>
    </div>

    <!-- ═══════════════ PROXY ═══════════════ -->
    <div class="page-section" id="sec-proxy">
      <div class="hero">
        <div class="hero-card green">
          <div class="hero-glow"></div>
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3fb950" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg></div>
          <div class="hero-label">Tokens saved</div>
          <div class="hero-value" id="px-tokens">—</div>
          <div id="px-tokens-trend" class="hero-trend" style="visibility:hidden;color:var(--green)"> </div>
          <div class="spark-wrap"><svg viewBox="0 0 120 28" preserveAspectRatio="none"><polyline id="sl-px-tokens" points=""/></svg></div>
        </div>
        <div class="hero-card green">
          <div class="hero-glow"></div>
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3fb950" stroke-width="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg></div>
          <div class="hero-label">Cost saved</div>
          <div class="hero-value" id="px-cost">—</div>
          <div class="hero-sub" id="px-cost-sub"></div>
        </div>
        <div class="hero-card neutral">
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7a8799" stroke-width="2"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></div>
          <div class="hero-label">Requests</div>
          <div class="hero-value" id="px-requests">—</div>
          <div class="hero-sub" id="px-compressed"></div>
          <div class="bar-wrap"><div class="bar" id="px-req-bar" style="width:0%;background:var(--blue)"></div></div>
        </div>
        <div class="hero-card blue">
          <div class="hero-glow"></div>
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#58a6ff" stroke-width="2"><polyline points="22 8 22 16"/><polyline points="2 8 2 16"/><path d="M5 5h14l2 3-9 4-9-4 2-3Z"/></svg></div>
          ...
          <div class="ring-wrap">
            <div class="ring">
              <svg width="56" height="56" viewBox="0 0 56 56"><circle class="ring-bg" cx="28" cy="28" r="22"/><circle class="ring-fill" id="px-ring-arc" cx="28" cy="28" r="22"/></svg>
              <div class="ring-label" id="px-ring-pct">0%</div>
            </div>
            <div><div class="hero-value" id="px-ratio">—</div><div class="hero-sub">smaller</div></div>
          </div>
        </div>
      </div>
      ...
      <div class="tbl-wrap">
        <table>
          <thead><tr><th>Compressor</th><th class="r">Samples</th><th class="r">Quality</th><th class="r">Numeric</th><th>Score</th></tr></thead>
          <tbody id="px-byc-body">
    ...
    <!-- ═══════════════ GATEWAY ═══════════════ -->
    <div class="page-section" id="sec-gateway">
      <div class="gw-hero">
        <div class="hero-card green">
          <div class="hero-glow"></div>
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#3fb950" stroke-width="2"><polyline points="23 6 13.5 15.5 8.5 10.5 1 18"/><polyline points="17 6 23 6 23 12"/></svg></div>
          <div class="hero-label">Tokens saved (est.)</div>
          <div class="hero-value" id="gw-tokens">—</div>
          <div class="spark-wrap"><svg viewBox="0 0 120 28" preserveAspectRatio="none"><polyline id="sl-gw-tokens" points=""/></svg></div>
        </div>
        <div class="hero-card neutral">
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#7a8799" stroke-width="2"><path d="M12 2H2v10l9.29 9.29c.94.94 2.48.94 3.42 0l6.58-6.58c.94-.94.94-2.48 0-3.42L12 2Z"/><path d="M7 7h.01"/></svg></div>
          <div class="hero-label">Tool calls</div>
          <div class="hero-value" id="gw-calls">—</div>
          <div class="hero-sub" id="gw-calls-sub"></div>
          <div class="bar-wrap"><div class="bar" id="gw-calls-bar" style="width:0%;background:var(--blue)"></div></div>
        </div>
        <div class="hero-card blue">
          <div class="hero-glow"></div>
          <div class="hero-icon"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#58a6ff" stroke-width="2"><polyline points="22 8 22 16"/><polyline points="2 8 2 16"/><path d="M5 5h14l2 3-9 4-9-4 2-3Z"/></svg></div>
          ...
          <div class="ring-wrap">
            <div class="ring">
              <svg width="56" height="56" viewBox="0 0 56 56"><circle class="ring-bg" cx="28" cy="28" r="22"/><circle class="ring-fill" id="gw-ring-arc" cx="28" cy="28" r="22"/></svg>
              <div class="ring-label" id="gw-ring-pct">0%</div>
            </div>
            <div><div class="hero-value" id="gw-ratio">—</div><div class="hero-sub">smaller</div></div>
          </div>
        </div>
      </div>
      <div class="section-head"><h2>Per-tool savings</h2></div>
      <div class="tbl-wrap">
        <table>
          <thead><tr><th>Server · tool</th><th class="r">Calls</th><th class="r">Before</th><th class="r">After</th><th class="r">Saved</th><th>Reduction</th></tr></thead>
    ...
    <!-- ═══════════════ QUALITY ═══════════════ -->
    <div class="page-section" id="sec-quality">
      <div class="q-grid">
        <div class="q-card">
          <div class="q-title">ROUGE-1 quality (A/B)</div>
          <div class="q-score hi" id="ql-quality">—</div>
          <div class="hero-sub">mean across samples</div>
          <div class="q-bar"><div class="q-bar-fill" id="ql-quality-bar" style="width:0%"></div></div>
        </div>
        <div class="q-card">
          <div class="q-title">Numeric consistency</div>
          <div class="q-score hi" id="ql-numeric">—</div>
          <div class="hero-sub">exact value preservation</div>
          <div class="q-bar"><div class="q-bar-fill" id="ql-numeric-bar" style="width:0%"></div></div>
        </div>
      </div>
      <div class="section-head"><h2>Per-compressor breakdown</h2></div>
      <div class="tbl-wrap">
        <table>
          <thead><tr><th>Compressor</th><th class="r">Samples</th><th class="r">Quality</th><th class="r">Numeric</th><th>Score</th></tr></thead>
          <tbody id="ql-byc-body">
            <tr><td colspan="5"><div class="empty-state">
              <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5"><path d="M9 17H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v5"/><path d="m19 12-7 7-4-4"/></svg>
              <p>No A/B samples yet — run the proxy with<br><code>--ab-sample-rate &gt; 0</code></p>
            </div></td></tr>
...
// ── Navigation ────────────────────────────────────────────────
const PAGE_TITLES = {overview:'Overview', proxy:'Proxy', gateway:'MCP Gateway', quality:'Quality'};
function switchPage(page) {
  document.querySelectorAll('.nav-item').forEach(el => {
    el.classList.toggle('active', el.dataset.page === page);
  });
  document.querySelectorAll('.page-section').forEach(el => {
    el.classList.toggle('visible', el.id === 'sec-' + page);
  });
...
function pushSpark(key, val) {
  if (!sparkHist[key]) sparkHist[key] = [];
  sparkHist[key].push(val);
  if (sparkHist[key].length > HIST) sparkHist[key].shift();
}
function drawSpark(polyId, data) {
  const el = $(polyId); if (!el || data.length < 2) return;
  const max = Math.max(...data) || 1, min = Math.min(...data);
  const W = 120, H = 28, pad = 2;
  el.setAttribute('points', data.map((v, i) => {
...
function setRing(arcId, pctId, val) {
  const offset = 138.2 * (1 - clamp(val, 0, 1));
  const arc = $(arcId); if (arc) arc.style.strokeDashoffset = offset.toFixed(2);
  const lbl = $(pctId); if (lbl) lbl.textContent = (val * 100).toFixed(0) + '%';
}
function setQScore(id, barId, val) {
  const el = $(id); if (!el) return;
  el.textContent = val != null ? val.toFixed(3) : '—';
  el.className = 'q-score ' + (val != null ? qClass(val) : 'hi');
  const bar = $(barId); if (bar) bar.style.width = val != null ? (val*100).toFixed(1)+'%' : '0%';
}
function bycRows(data, bodyId) {
  const body = $(bodyId); if (!body) return;
  const rows = Object.entries(data || {});
  if (!rows.length) return;
  body.innerHTML = rows.sort((a,b) => b[1].samples - a[1].samples).map(([name,c],i) => {
    const qv = c.mean_quality ?? 0, nv = c.mean_numeric_consistency ?? 1;
    const col = COLORS[i % COLORS.length];
    const qcl = qv>=.8?'g':qv>=.5?'w':'m';
    return '<tr>'
      ...
      + '<td><div class="pbar-wrap"><div class="pbar '+qcl+'" style="width:'+clamp(qv*100,0,100)+'px"></div>'
...
function fmtUp() {
  ...
  return h ? 'session '+h+'h '+min+'m' : m ? 'session '+m+'m '+sec+'s' : 'session '+sec+'s';
}
setInterval(() => { const u=fmtUp(); $('uptime').textContent=u; $('uptime2').textContent=u; }, 1000);
...
async function tick() {
  try {
    ...
    const costSub = (serverDollars != null && serverDollars > 0)
      ? 'server-side pricing' : (price ? 'at $'+price+'/1M tokens' : '');

    pushSpark('tokens', tokens);
    drawSpark('sl-tokens',    sparkHist.tokens);
    drawSpark('sl-px-tokens', sparkHist.tokens);
    ...
    const trendEl = (id) => {
      ...
      if (prevTokens !== null && tokens !== prevTokens) {
        ...
        el.textContent = (delta>0?'↑ +':'↓ ')+fmt(Math.abs(delta))+' this interval';
      }
    };
    trendEl('ov-tokens-trend'); trendEl('px-tokens-trend');
    prevTokens = tokens;

    // overview
    animateTo('ov-tokens',   tokens, fmt);
    animateTo('ov-requests', total,  fmt);
    $('ov-cost').textContent = '$'+costVal;
    $('ov-cost-sub').textContent = costSub;
    $('ov-compressed').textContent = comp ? fmt(comp)+' compressed' : '';
    $('ov-req-bar').style.width = total ? pct(clamp(comp/total,0,1)) : '0%';
    $('ov-ratio').textContent = pct(saved);
    setRing('ov-ring-arc','ov-ring-pct',saved);
    setQScore('ov-quality','ov-quality-bar',q.quality?.mean);
    setQScore('ov-numeric','ov-numeric-bar',q.numeric_consistency?.mean);

    // proxy
    animateTo('px-tokens',   tokens, fmt);
    animateTo('px-requests', total,  fmt);
    $('px-cost').textContent = '$'+costVal;
    $('px-cost-sub').textContent = costSub;
    $('px-compressed').textContent = comp ? fmt(comp)+' compressed' : '';
    $('px-req-bar').style.width = total ? pct(clamp(comp/total,0,1)) : '0%';
    $('px-ratio').textContent = pct(saved);
    setRing('px-ring-arc','px-ring-pct',saved);
    bycRows(q.by_compressor, 'px-byc-body');

    // quality
    setQScore('ql-quality','ql-quality-bar',q.quality?.mean);
    setQScore('ql-numeric','ql-numeric-bar',q.numeric_consistency?.mean);
    bycRows(q.by_compressor, 'ql-byc-body');
    ...
    pushSpark('gtokens', gtok);
    drawSpark('sl-gtokens',    sparkHist.gtokens);
    drawSpark('sl-gw-tokens',  sparkHist.gtokens);

    animateTo('ov-g-tokens', gtok,   fmt);
    animateTo('ov-g-calls',  gcalls, fmt);
    $('ov-g-calls-sub').textContent = gcomp ? fmt(gcomp)+' compressed' : '';
    $('ov-g-ratio').textContent = pct(gsaved);

    animateTo('gw-tokens', gtok,   fmt);
    animateTo('gw-calls',  gcalls, fmt);
    $('gw-calls-sub').textContent = gcomp ? fmt(gcomp)+' compressed' : '';
    $('gw-calls-bar').style.width = gcalls ? pct(clamp(gcomp/gcalls,0,1)) : '0%';
    $('gw-ratio').textContent = pct(gsaved);
    setRing('gw-ring-arc','gw-ring-pct',gsaved);

    const toolRows = Object.entries(g.by_tool || {});
    if (toolRows.length) {
      $('bytool-body').innerHTML = toolRows
